...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: xeq.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: xeqd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeXEQamount=<n> " + _("Keep N XEQ anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. XEQ is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. XEQ is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("XEQ version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "XEQ server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of XEQ") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart XEQ to complete") << "\n";
    ...
    nAnonymizeXEQAmount = GetArg("-anonymizexeqamount", 0);
    if(nAnonymizeXEQAmount > 999999) nAnonymizeXEQAmount = 999999;
    if(nAnonymizeXEQAmount < 2) nAnonymizeXEQAmount = 2;
    ...
    LogPrintf("Anonymize XEQ Amount %d\n", nAnonymizeXEQAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XEQ+1000 == (.1XEQ+100)*10
       10XEQ+10000 == (1XEQ+1000)*10
    */
