...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXEQAmount"))
        settings.setValue("nAnonymizeXEQAmount", 1000);
    nAnonymizeXEQAmount = settings.value("nAnonymizeXEQAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeXEQAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeXEQAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeXEQAmount:
            return QVariant(nAnonymizeXEQAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeXEQAmount:
            nAnonymizeXEQAmount = value.toInt();
            settings.setValue("nAnonymizeXEQAmount", nAnonymizeXEQAmount);
            emit AnonymizeXEQAmountChanged(nAnonymizeXEQAmount);
            break;
        default:
            break;
        }
