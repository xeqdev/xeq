...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XEQ
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XEQ
    // Mac: ~/Library/Application Support/XEQ
    // Unix: ~/.xeq
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XEQ";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "xeq.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "xeqd.pid"));
